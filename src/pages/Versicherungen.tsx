  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-navy-950">Versicherungen</h1>
          <p className="text-navy-500 text-sm mt-1">Monatliche Kosten: {formatEuro(gesamt)}</p>
        </div>
      ...
      {daten.versicherungen.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 shadow-sm border border-gray-100 text-center">
          ...
          <p className="text-sm text-navy-400 mb-4">Behalte den Überblick über all deine Policen</p>
            ...
            <div key={v.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-navy-400">{VERSICHERUNG_TYPEN[v.typ]}</p>
                  </div>
                </div>
                <button onClick={() => handleDelete(v.id)} className="p-2 hover:bg-red-50 rounded-lg transition-colors">
              ...
              <div className="space-y-1.5 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-navy-900">{v.anbieter}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-navy-500">Beitrag</span>
                  <span className="font-medium text-navy-900">{formatEuro(v.betrag)} / {INTERVALLE[v.intervall]?.toLowerCase()}</span>
                </div>
                {v.intervall !== 'monatlich' && (
                  <div className="flex justify-between">
                    <span className="text-navy-500">Monatlich</span>
                    <span className="font-medium text-navy-900">{formatEuro(monatlichBetrag(v.betrag, v.intervall))}</span>
                  </div>
                )}
                {v.vertragsnummer && (
                  <div className="flex justify-between">
                    <span className="text-navy-500">Vertragsnr.</span>
                    <span className="font-medium text-navy-900">{v.vertragsnummer}</span>
                  </div>
                )}
                {v.enddatum && (
                  <div className="flex justify-between">
                    <span className="text-navy-500">Enddatum</span>
                    <span className="font-medium text-navy-900">{new Date(v.enddatum).toLocaleDateString('de-DE')}</span>
                  </div>
                )}
              </div>
            </div>
...
function VersicherungForm({ onSubmit }: { onSubmit: (v: Versicherung) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-navy-700 mb-1">Anbieter</label>
        <input type="text" value={anbieter} onChange={e => setAnbieter(e.target.value)} placeholder="z.B. Allianz"
          className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-navy-500 focus:border-transparent" />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-sm font-medium text-navy-700 mb-1">Beitrag</label>
      ...
      <div>
        <label className="block text-sm font-medium text-navy-700 mb-1">Typ</label>
      ...
      <div>
        <label className="block text-sm font-medium text-navy-700 mb-1">Vertragsnummer (optional)</label>
        <input type="text" value={vertragsnummer} onChange={e => setVertragsnummer(e.target.value)}
          className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-navy-500 focus:border-transparent" />
      </div>
      <div>
        <label className="block text-sm font-medium text-navy-700 mb-1">Vertragsende (optional)</label>
