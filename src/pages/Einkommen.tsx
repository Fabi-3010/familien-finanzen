  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-navy-950">Einkommen</h1>
          <p className="text-navy-500 text-sm mt-1">Monatliches Einkommen: {formatEuro(gesamt)}</p>
        </div>
        <button
      ...
      {daten.einkommen.length === 0 ? (
        ...
        <div className="space-y-3">
          {daten.einkommen.map(e => (
            <div key={e.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-navy-400">{EINKOMMEN_TYPEN[e.typ]} · {e.person}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-lg font-bold text-emerald-600">{formatEuro(e.betrag)}</span>
                <button onClick={() => handleDelete(e.id)} className="p-2 hover:bg-red-50 rounded-lg transition-colors">
...
function EmptyState({ onAdd }: { onAdd: () => void }) {
  return (
    <div className="bg-white rounded-2xl p-12 shadow-sm border border-gray-100 text-center">
      ...
      <p className="text-sm text-navy-400 mb-4">Füge dein monatliches Einkommen hinzu</p>
...
function EinkommenForm({ personen, onSubmit }: { personen: string[]; onSubmit: (e: EinkommenTyp) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-navy-700 mb-1">Betrag (monatlich)</label>
